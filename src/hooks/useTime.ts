...
export default function useTime() {
    ...
    useLayoutEffect(() => {
        ...
        // 开启定时器
        if (time === 0) {
            setHash(new Date(new Date().getTime() + ((new Date().getTimezoneOffset() / 60) + 8) * 3600 * 1000).getTime())
    ...
    const init = useCallback(
        () => {
            ...
            if (!!Number(group.endTime) && !!Number(group.startTime) && !!Number(group.nowTime)) {
                const newTime = group.nowTime
                // const newTime = new Date(new Date().getTime() + ((new Date().getTimezoneOffset() / 60) + 8) * 3600 * 1000).getTime()
                let timeNum = 0
                if (!status) {
                    timeNum = Math.floor((Number(group.startTime) - Number(newTime)) / 1000)
                    if (timeNum <= 0) {
                        setHash(newTime)
                    }
                    setTime(timeNum)
