...
export default function useUSDTGroup(contractAddress: AddressType, coinName: CoinAddressType) {
    ...
    /**
      *  TOKENBalance 余额
      */
    const initTOKENBalance = useCallback(
        async () => {
            if (!!account) {
    ...
    const handleApprove = useCallback(
        async () => {
            ...
            if (account) {
                showLoding(true)
                try {
                    const res = await Contracts.example?.approve(account, contractAddress, coinName)
                    console.log("res", res)
    ...
    *  TOKENBalance 授权额度
    */
    const initTOKENAllowance = useCallback(
        async () => {
            if (!!account) {
                const allowance = await Contracts.example?.Tokenapprove(account, contractAddress, coinName)
   ...
   */
    ...
    */
    const handleTransaction = useCallback(
        ...
        async (price: number | string, transactionCallBack: (refreshCall: () => void) => void) => {
        ...
        [account, handleApprove, symbol, handleUSDTRefresh, coinName, TOKENBalance, TOKENAllowance, addMessage]
    )
